...
export default function PropertiesPanel() {
  ...
  const reportData = useMemo(() => {
    if (!placements || typeof placements !== 'object') return { rows: [], totalCount: 0, totalValue: 0, totalProfit: 0 }
  ...
  return (
    <div className={`fixed bottom-4 right-4 ${minimized ? 'w-[480px] p-3 px-5' : 'w-[580px] p-5'} bg-glass-bg border border-glass-border backdrop-blur-xl rounded-3xl z-30 shadow-3xl animate-in fade-in slide-in-from-right-4 duration-700 transition-all duration-300`}>
      ...
      <div className={`grid grid-cols-3 gap-4 relative ${minimized ? '' : 'pb-4 border-b border-white/10'}`}>
        <StatItem label="Product Value" value={`$${reportData.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} icon={<TrendingUp size={12} className="text-accent flex-shrink-0" />} />
        <StatItem label="Product Count" value={reportData.totalCount} icon={<Package size={12} className="text-secondary flex-shrink-0" />} />
        <StatItem label="Total Profit" value={`$${reportData.totalProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} icon={<DollarSign size={12} className="text-emerald-400 flex-shrink-0" />} isProfit profitValue={reportData.totalProfit} />
      </div>
                ...
                {reportData.rows.length > 0 ? (
                  reportData.rows.map((row) => (
                    <tr key={row.product.id} className="group transition-all hover:translate-x-0.5">
                      ...
                      <td className="px-2 py-2 bg-black/5 first:rounded-l-xl last:rounded-r-xl border-y border-black/5 group-hover:bg-black/10">
                        <PriceInput value={unitPrices[row.product.id]} onChange={val => setUnitPrices(p => ({ ...p, [row.product.id]: val }))} accent="accent" />
                      </td>
                      <td className="px-2 py-2 bg-black/5 first:rounded-l-xl last:rounded-r-xl border-y border-black/5 group-hover:bg-black/10">
                        <PriceInput value={unitCosts[row.product.id]} onChange={val => setUnitCosts(p => ({ ...p, [row.product.id]: val }))} accent="secondary" />
                      </td>
                      <td className="px-2 py-2 bg-black/5 first:rounded-l-xl last:rounded-r-xl border-y border-black/5 group-hover:bg-black/10">
...
function StatItem({ label, value, icon, isProfit, profitValue }) {
  return (
    <div className="flex flex-col gap-1 border-r last:border-r-0 border-white/5 py-1">
      <div className="flex items-center gap-1.5 opacity-50">
        {icon}
        <span className="text-[10px] font-black uppercase tracking-widest text-text-dim whitespace-nowrap">{label}</span>
      </div>
      <span className={`text-lg font-black tabular-nums leading-none ${isProfit ? (profitValue >= 0 ? 'text-emerald-400' : 'text-red-400') : 'text-text-main'}`}>
        {value}
      </span>
    </div>
  )
